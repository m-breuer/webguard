...
<div
    {{ $attributes->class('relative')->merge(['data-select-control' => 'multi']) }}
    x-data="{
        open: false,
        query: '',
...
>
    ...
    <div
        class="flex min-h-11 w-full items-center gap-2 rounded-md border border-purple-200 bg-purple-50/40 px-3 py-2 text-sm shadow-sm transition focus-within:border-purple-500 focus-within:ring-2 focus-within:ring-purple-500/30 focus-within:outline-none dark:border-purple-800 dark:bg-purple-950/30 dark:text-gray-100"
        x-on:click="
            open = true;
    ...
    >
            ...
            <template x-for="option in selectedOptions" :key="option.value">
                <span class="inline-flex max-w-full items-center gap-1 rounded-md bg-purple-100 px-2 py-1 font-medium text-purple-800 dark:bg-purple-900/70 dark:text-purple-100">
                    ...
                    <button
                        ...
                        x-bind:aria-label="removeLabel + ' ' + option.label"
                    >
                        &times;
                    </button>
                </span>
        ...
        <div class="flex shrink-0 items-center gap-2 text-purple-600 dark:text-purple-300">
            <button
                ...
                class="rounded-sm px-1 text-lg leading-none hover:text-gray-700 focus:ring-2 focus:ring-purple-500/30 focus:outline-none dark:hover:text-gray-100"
            ...
            <button
                type="button"
                class="rounded-sm p-0.5 hover:text-gray-700 focus:ring-2 focus:ring-purple-500/30 focus:outline-none dark:hover:text-gray-100"
                x-on:click.stop="
                    open = ! open;
                    $nextTick(() => $refs.search?.focus());
                "
    ...
    >
        <button
            type="button"
            class="flex w-full items-center gap-2 px-3 py-2 text-left font-medium text-gray-700 hover:bg-gray-100 focus:bg-gray-100 focus:outline-none disabled:cursor-not-allowed disabled:opacity-60 dark:text-gray-200 dark:hover:bg-gray-800 dark:focus:bg-gray-800"
