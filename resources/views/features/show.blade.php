            ...
            <x-main class="grid w-full gap-10 py-14 lg:grid-cols-[1fr_0.48fr] lg:items-center lg:py-20">
                <div>
                    <a href="{{ route('public-features.index') }}"
                        class="inline-flex text-sm font-semibold text-emerald-700 transition hover:text-emerald-800 dark:text-emerald-300 dark:hover:text-emerald-200">
                        &lt;- {{ __('public_features.show.back') }}
                    </a>
                    <x-paragraph class="mt-6 text-sm font-semibold uppercase tracking-[0.1em] text-emerald-700 dark:text-emerald-300">
                ...
                <aside class="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-lg shadow-slate-300/20 dark:border-slate-800 dark:bg-slate-900/70 dark:shadow-slate-950/20">
                    <div class="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700 dark:bg-emerald-300/15 dark:text-emerald-300">
                    ...
                    <ul class="mt-4 space-y-3 text-sm leading-7 text-slate-700 dark:text-slate-300">
                        @foreach ($highlights as $highlight)
                            <li class="flex gap-3">
                                <span class="mt-2 h-2 w-2 flex-none rounded-full bg-emerald-500"></span>
                                <span>{{ $highlight }}</span>
                            </li>
                        @endforeach
                    </ul>
            ...
            <x-main class="grid w-full gap-8 lg:grid-cols-[1fr_0.36fr]">
                <div class="space-y-6">
                    ...
                    @if (in_array($featureKey, ['rest_api', 'server_health'], true))
                        <section class="rounded-3xl border border-emerald-300/70 bg-emerald-50/80 p-8 shadow-sm dark:border-emerald-300/30 dark:bg-emerald-300/10 sm:p-10">
                            <x-heading type="h2" class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">
                            ...
                            <x-primary-button :href="route('scribe')"
                                class="mt-6 bg-emerald-500 px-6 py-3 text-base font-semibold text-white normal-case tracking-normal shadow-lg shadow-emerald-500/20 transition hover:bg-emerald-600 focus:ring-emerald-500 dark:bg-emerald-400 dark:text-slate-950 dark:hover:bg-emerald-300 dark:focus:ring-emerald-300">
                                {{ __('public_features.common.api_docs') }}
                            </x-primary-button>
                        </section>
                    @endif
                            ...
                            @foreach ($feature['related'] as $relatedSlug)
                                @if ($relatedSlug === 'monitoring-locations')
                                    <a href="{{ route('monitoring-locations') }}"
                                        class="rounded-2xl border border-slate-200 bg-slate-50 p-5 transition hover:border-emerald-300 dark:border-slate-800 dark:bg-slate-950/60 dark:hover:border-emerald-400/50">
                            ...
                            @foreach ($features as $otherSlug => $otherFeature)
                                <a href="{{ route('public-features.show', $otherSlug) }}"
                                    @class([
                                        ...
                                        'bg-emerald-100 text-emerald-800 dark:bg-emerald-300/15 dark:text-emerald-200' => $otherSlug === $slug,
                                        'text-slate-700 hover:bg-slate-100 hover:text-emerald-700 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-emerald-300' => $otherSlug !== $slug,
                                    ])>
