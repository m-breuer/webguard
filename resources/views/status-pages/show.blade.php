    ...
    <x-slot name="header">
        <div data-status-page-detail-header>
            <a
                ...
                class="mb-3 inline-flex items-center gap-2 text-sm font-semibold text-purple-700 hover:text-purple-900 dark:text-purple-300 dark:hover:text-purple-200"
            >
            ...
            <div class="flex flex-wrap items-start justify-between gap-4">
                ...
                >
                    <x-secondary-button
                        ...
                        :icon-only="true"
                        title="{{ __('incidents.analytics.link') }}"
                        aria-label="{{ __('incidents.analytics.link') }}"
                    >
        ...
        <div data-status-page-detail-layout class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
            <div class="min-w-0 space-y-4">
                ...
                <x-container>
                    ...
                    <form
                        method="GET"
                        action="{{ route('status-pages.show', $statusPage) }}"
                        class="mt-4 grid gap-3 rounded-md border border-gray-200 p-3 md:grid-cols-3 dark:border-gray-700"
                    >
                        <div>
                            <x-input-label
                                for="follow-up-status-filter"
                                :value="__('status_page.incident_follow_ups.filters.status')"
                            />
                            <x-select-input id="follow-up-status-filter" name="follow_up_status" class="mt-1 w-full">
                                ...
                                @foreach ($followUpStatuses as $followUpStatus)
                                    <option
                                        ...
                                        @selected($followUpFilters['status'] === $followUpStatus->value)
                                    >
                        ...
                        <div>
                            <x-input-label
                                for="follow-up-assignee-filter"
                                :value="__('status_page.incident_follow_ups.filters.assignee')"
                            />
                            <x-select-input
                            ...
                            >
                                <option value="">{{ __('status_page.incident_follow_ups.filters.all') }}</option>
                                <option
                                    value="{{ $statusPage->user_id }}"
                                    @selected($followUpFilters['assignee'] === $statusPage->user_id)
                                >
                                    {{ $statusPage->user->name }}
                                </option>
                            </x-select-input>
                        </div>
                        ...
                        <div class="mt-4 divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach ($incidents as $incident)
                                <div id="incident-{{ $incident->id }}" class="space-y-4 py-4">
                                    ...
                                    <details
                                        id="incident-workbench-{{ $incident->id }}"
                                        @if ($openIncidentId === $incident->id || $incident->up_at === null) open @endif
                                        class="rounded-lg border border-slate-200 bg-slate-50/60 p-3 dark:border-slate-700 dark:bg-slate-950/20"
                                    >
                                            ...
                                            @if (! Auth::user()->isDemo())
                                                ...
                                                >
                                                    ...
                                                    <div class="grid gap-3 md:grid-cols-2">
                                                        <div>
                                                            ...
                                                            >
                                                                <option value="">
                                                                    {{ __('incidents.analytics.filters.all') }}
                                                                </option>
                                                                @foreach (\App\Enums\IncidentType::cases() as $incidentType)
                                                                    <option
                                                                        value="{{ $incidentType->value }}"
                                                                        @selected(old('incident_type', $incident->incident_type?->value) === $incidentType->value)
                                                                    >
                                                        ...
                                                        <div>
                                                            <x-input-label
                                                                for="incident-severity-{{ $incident->id }}"
                                                            ...
                                                            >
                                                                ...
                                                                @foreach (\App\Enums\IncidentSeverity::cases() as $severity)
                                                                    <option
                                                                        ...
                                                                        @selected(old('severity', $incident->severity?->value) === $severity->value)
                                                                    >
                                                        ...
                                                        <div>
                                                            <x-input-label
                                                                for="incident-service-{{ $incident->id }}"
                                                                :value="__('status_page.incident_metadata.affected_service')"
                                                            />
                                                            <x-text-input
                                                                id="incident-service-{{ $incident->id }}"
                                                        ...
                                                        <div>
                                                            <x-input-label
                                                                for="incident-impact-{{ $incident->id }}"
                                                            ...
                                                            >
                                                                ...
                                                                @foreach (\App\Enums\IncidentCustomerImpact::cases() as $impact)
                                                                    <option
                                                                        value="{{ $impact->value }}"
                                                                        @selected(old('customer_impact', $incident->customer_impact?->value) === $impact->value)
                                                                    >
                                                        ...
                                                        <div class="md:col-span-2">
                                                            <x-input-label
                                                                for="incident-category-{{ $incident->id }}"
                                                                :value="__('status_page.incident_metadata.contributing_category')"
                                                            />
                                                            <x-select-input
                                                                id="incident-category-{{ $incident->id }}"
                                                                name="contributing_category"
                                                                class="mt-1 w-full"
                                                            >
                                                                <option value="">
                                                                    {{ __('incidents.analytics.filters.all') }}
                                                                ...
                                                                @foreach (\App\Enums\IncidentContributingCategory::cases() as $category)
                                                                    <option
                                                                        ...
                                                                        @selected(old('contributing_category', $incident->contributing_category?->value) === $category->value)
                                                                    >
                                                ...
                                                <div class="space-y-3 rounded-md border border-amber-200 bg-amber-50/50 p-3 dark:border-amber-900 dark:bg-amber-950/20">
                                                    ...
                                                    @foreach ($incident->followUps as $followUp)
                                                        <form
                                                            method="POST"
                                                            action="{{ route('status-pages.incident-follow-ups.update', [$statusPage, $incident, $followUp]) }}"
                                                            ...
                                                            <div class="grid gap-2 md:grid-cols-2">
                                                                ...
                                                                <x-select-input name="status" class="w-full">
                                                                    @foreach (\App\Enums\IncidentFollowUpStatus::cases() as $followUpStatus)
                                                                        <option
                                                                            value="{{ $followUpStatus->value }}"
                                                                            @selected($followUp->status === $followUpStatus)
                                                                        >
                                                                            {{ __('status_page.incident_follow_ups.statuses.' . $followUpStatus->value) }}
                                                                        </option>
                                                                    @endforeach
                                                                ...
                                                                <x-text-input
                                                                    type="date"
                                                                    name="due_at"
                                                                    :value="$followUp->due_at?->format('Y-m-d')"
                                                                    class="w-full"
                                                                />
                                                                <x-text-input
                                                                    name="external_url"
                                                                    :value="$followUp->external_url"
                                                                    class="w-full"
                                                                />
                                                            </div>
                                                            <x-textarea
                                                                name="description"
                                                                rows="2"
                                                            >{{ $followUp->description }}</x-textarea>
                                                            <div class="flex flex-wrap gap-2">
                                                                <x-primary-button>{{ __('status_page.incident_follow_ups.save') }}</x-primary-button>
                                                                <button
                                                                    type="submit"
                                                                    form="delete-follow-up-{{ $followUp->id }}"
                                                                    class="inline-flex h-10 w-10 items-center justify-center rounded-md text-red-600 transition hover:bg-red-50 focus:ring-2 focus:ring-red-500 focus:outline-hidden dark:text-red-400 dark:hover:bg-red-950/30"
                                                        ...
                                                        <form
                                                            id="delete-follow-up-{{ $followUp->id }}"
                                                            method="POST"
                                                    ...
                                                    <form
                                                        method="POST"
                                                        action="{{ route('status-pages.incident-follow-ups.store', [$statusPage, $incident]) }}"
                                                        class="space-y-2"
                                                    >
                                                        @csrf
                                                        <div class="grid gap-2 md:grid-cols-2">
                                                            <x-text-input
                                                                name="title"
                                                                :placeholder="__('status_page.incident_follow_ups.title')"
                                                                class="w-full"
                                                            />
                                                            <x-text-input
                                                                type="date"
                                                                name="due_at"
                                                                :placeholder="__('status_page.incident_follow_ups.due_at')"
                                                                class="w-full"
                                                            />
                                                            <x-text-input
                                                                name="external_url"
                                                                :placeholder="__('status_page.incident_follow_ups.external_url')"
                                                                class="w-full"
                                                            />
                                                            <x-select-input name="assigned_user_id" class="w-full">
                                                                <option value="">
                                                                    {{ __('status_page.incident_follow_ups.unassigned') }}
                                                                </option>
                                                                <option value="{{ $statusPage->user_id }}">
                                                                    {{ $statusPage->user->name }}
                                                                </option>
                                                            </x-select-input>
                                                        </div>
                                                        <x-textarea
                                                            name="description"
                                                            rows="2"
                                                            :placeholder="__('status_page.incident_follow_ups.description_field')"
                                                        ></x-textarea>
                                                ...
                                                <div class="space-y-3 rounded-md border border-green-200 bg-green-50/50 p-3 dark:border-green-900 dark:bg-green-950/20">
                                                    ...
                                                    @foreach ($incidentTimelines[$incident->id] ?? [] as $timelineEvent)
                                                        <div class="rounded-md border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-800">
                                                            <div class="flex flex-wrap items-center justify-between gap-2">
                                                                ...
                                                                <x-date-time
                                                                    :value="$timelineEvent['occurred_at']"
                                                                    class="text-sm text-gray-500 dark:text-gray-400"
                                                                />
                                                            </div>
                                                            <p class="mt-1 text-xs tracking-wide text-gray-500 uppercase dark:text-gray-400">
                                                            ...
                                                            @if ($timelineEvent['id'])
                                                                <form
                                                                    method="POST"
                                                                    action="{{ route('status-pages.incident-timeline.update', [$statusPage, $incident, $timelineEvent['id']]) }}"
                                                                ...
                                                                >
                                                                    ...
                                                                    <div class="grid gap-2 md:grid-cols-2">
                                                                        ...
                                                                        <x-text-input
                                                                            type="datetime-local"
                                                                            name="occurred_at"
                                                                            :value="$timelineEvent['occurred_at']->format('Y-m-d\\TH:i')"
                                                                            class="w-full"
                                                                        />
                                                                    </div>
                                                                    <x-textarea
                                                                        name="description"
                                                                        rows="2"
                                                                    >{{ $timelineEvent['description'] }}</x-textarea>
                                                                    <div class="flex flex-wrap gap-2">
                                                                        <x-primary-button>{{ __('status_page.incident_timeline.save') }}</x-primary-button>
                                                                        <button
                                                                            type="submit"
                                                                            form="delete-timeline-{{ $timelineEvent['id'] }}"
                                                                            class="inline-flex h-10 w-10 items-center justify-center rounded-md text-red-600 transition hover:bg-red-50 focus:ring-2 focus:ring-red-500 focus:outline-hidden dark:text-red-400 dark:hover:bg-red-950/30"
                                                                ...
                                                                <form
                                                                    id="delete-timeline-{{ $timelineEvent['id'] }}"
                                                                    method="POST"
                                                    ...
                                                    <form
                                                        method="POST"
                                                        action="{{ route('status-pages.incident-timeline.store', [$statusPage, $incident]) }}"
                                                        class="space-y-2"
                                                    >
                                                        @csrf
                                                        <div class="grid gap-2 md:grid-cols-2">
                                                            <x-text-input
                                                                name="title"
                                                                :placeholder="__('status_page.incident_timeline.title')"
                                                                class="w-full"
                                                            />
                                                            <x-text-input
                                                                type="datetime-local"
                                                        ...
                                                        <x-textarea
                                                            name="description"
                                                            rows="2"
                                                            :placeholder="__('status_page.incident_timeline.description_field')"
                                                        ></x-textarea>
                                    ...
                                    @if (! Auth::user()->isDemo())
                                        ...
                                        >
                                            ...
                                            <div>
                                                <x-input-label
                                                    for="incident-problem-{{ $incident->id }}"
                                                ...
                                                <x-textarea
                                                    ...
                                                    rows="3"
                                                    :placeholder="__('status_page.incident_review.problem_placeholder')"
                                                >{{ old('problem_description') ?: $incident->problem_description }}</x-textarea>
                                                <x-input-error :messages="$errors->get('problem_description')" />
                                            </div>
                                            <div>
                                                <x-input-label
                                                    for="incident-resolution-{{ $incident->id }}"
                                                ...
                                                <x-textarea
                                                    ...
                                                    rows="3"
                                                    :placeholder="__('status_page.incident_review.resolution_placeholder')"
                                                >{{ old('resolution_description') ?: $incident->resolution_description }}</x-textarea>
                                    ...
                                    @if (! Auth::user()->isDemo())
                                        ...
                                        >
                                            ...
                                            <div>
                                                <x-input-label
                                                    for="incident-update-message-{{ $incident->id }}"
                                                    :value="__('status_page.incident_updates.message')"
                                                />
                                                <x-textarea
                                                    id="incident-update-message-{{ $incident->id }}"
                                                    name="message"
                                                    rows="3"
                ...
                <x-container>
                    ...
                    @if ($statusPage->is_public)
                        <a
                            ...
                            target="_blank"
                            rel="noopener"
                            class="mt-4 inline-flex h-10 w-10 items-center justify-center rounded-md border border-purple-300 text-purple-700 hover:bg-purple-50 focus:ring-2 focus:ring-purple-500 focus:outline-hidden dark:border-purple-700 dark:text-purple-300 dark:hover:bg-purple-950/30"
                ...
                <x-container>
                    ...
                    <div class="mt-4 space-y-3 text-sm">
                        <div class="rounded-lg bg-purple-50 p-3 text-purple-900 dark:bg-purple-950/30 dark:text-purple-100">
                            ...
                            <p class="mt-1 text-purple-800/80 dark:text-purple-200/80">
                                {{ __('status_page.incident_review.problem') }}
                            </p>
                        </div>
                        <div class="rounded-lg bg-gray-50 p-3 text-gray-700 dark:bg-gray-900/50 dark:text-gray-300">
                            ...
                            <p class="mt-1 text-gray-500 dark:text-gray-400">
                                {{ __('status_page.detail.regions_activity') }}
                            </p>
                        </div>
                    </div>
                </x-container>
