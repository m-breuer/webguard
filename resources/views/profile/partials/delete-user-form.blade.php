...
<x-paragraph> {{ __('profile.delete_account.description') }} </x-paragraph>
...
<x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
    <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
        ...
        <x-paragraph> {{ __('profile.delete_account.confirmation_warning') }} </x-paragraph>

        <x-input-label for="password" value="{{ __('profile.form.password') }}" class="sr-only" />

        <x-text-input id="password" name="password" type="password" placeholder="{{ __('profile.fields.password') }}" />
