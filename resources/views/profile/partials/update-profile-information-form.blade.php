...
<form method="post" action="{{ route('profile.update') }}" class="space-y-6">
    ...
    <section class="space-y-4">
        ...
        <div>
                ...
                <div>
                    <x-paragraph>
                        ...
                        <button
                            ...
                            class="inline-flex h-8 w-8 items-center justify-center rounded-md text-gray-600 hover:bg-gray-100 hover:text-gray-900 focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:outline-hidden dark:text-gray-300 dark:hover:bg-gray-700"
            ...
            @else
                <div>
                    <x-paragraph class="text-green-600"> {{ __('profile.messages.email_verified') }} </x-paragraph>
                </div>
            @endif
        </div>
    </section>
    ...
    <section class="space-y-4 border-t border-gray-200 pt-6 dark:border-gray-700">
        ...
        <div class="w-full md:w-1/2" x-data="{ theme: '{{ old('theme', $user->theme) }}' }">
            ...
            <x-select-input id="theme" class="block w-full" name="theme" @change="theme = $event.target.value">
                ...
                <option value="system" :selected="theme === 'system'">{{ __('profile.fields.theme_system') }}</option>
            </x-select-input>
    ...
    <section class="space-y-6 border-t border-gray-200 pt-6 dark:border-gray-700">
        ...
        <div class="space-y-4">
            ...
            <div class="rounded-xl border border-gray-200 bg-gray-50/60 p-5 shadow-xs dark:border-gray-700 dark:bg-gray-900/30">
                <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <x-text-checkbox
                        id="notification_channels_slack_enabled"
                        name="notification_channels[slack][enabled]"
                        :checked="(bool) data_get($notificationChannels, 'slack.enabled', false)"
                        :label="__('profile.notification_settings.enabled')"
                    />
                    <x-secondary-button
                        form="test-slack-notification-channel"
                        :icon-only="true"
                        title="{{ __('profile.notification_settings.test.action') }}"
                        aria-label="{{ __('profile.notification_settings.test.action') }}"
                    >
                        <x-icon name="send" class="h-4 w-4" />
                    </x-secondary-button>
                </div>
                ...
                <div class="mt-4">
                    <x-input-label
                        for="notification_channels_slack_webhook_url"
                        :value="__('profile.notification_settings.fields.slack_webhook_url')"
                    />
                    <x-text-input
                        id="notification_channels_slack_webhook_url"
                        name="notification_channels[slack][webhook_url]"
                        type="url"
                        :value="data_get($notificationChannels, 'slack.webhook_url')"
            ...
            <div class="rounded-xl border border-gray-200 bg-gray-50/60 p-5 shadow-xs dark:border-gray-700 dark:bg-gray-900/30">
                <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <x-text-checkbox
                        id="notification_channels_telegram_enabled"
                        name="notification_channels[telegram][enabled]"
                        :checked="(bool) data_get($notificationChannels, 'telegram.enabled', false)"
                        :label="__('profile.notification_settings.enabled')"
                    />
                    <x-secondary-button
                        form="test-telegram-notification-channel"
                        :icon-only="true"
                        title="{{ __('profile.notification_settings.test.action') }}"
                        aria-label="{{ __('profile.notification_settings.test.action') }}"
                    >
                ...
                <div class="mt-4 grid gap-4 md:grid-cols-2">
                    <div>
                        <x-input-label
                            for="notification_channels_telegram_bot_token"
                            :value="__('profile.notification_settings.fields.telegram_bot_token')"
                        />
                        <x-text-input
                            id="notification_channels_telegram_bot_token"
                            name="notification_channels[telegram][bot_token]"
                            type="text"
                    ...
                    <div>
                        <x-input-label
                            for="notification_channels_telegram_chat_id"
                            :value="__('profile.notification_settings.fields.telegram_chat_id')"
                        />
                        <x-text-input
                            id="notification_channels_telegram_chat_id"
                            name="notification_channels[telegram][chat_id]"
                            type="text"
            ...
            <div class="rounded-xl border border-gray-200 bg-gray-50/60 p-5 shadow-xs dark:border-gray-700 dark:bg-gray-900/30">
                <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <x-text-checkbox
                        id="notification_channels_discord_enabled"
                        name="notification_channels[discord][enabled]"
                        :checked="(bool) data_get($notificationChannels, 'discord.enabled', false)"
                        :label="__('profile.notification_settings.enabled')"
                    />
                    <x-secondary-button
                        form="test-discord-notification-channel"
                        :icon-only="true"
                        title="{{ __('profile.notification_settings.test.action') }}"
                        aria-label="{{ __('profile.notification_settings.test.action') }}"
                    >
                ...
                <div class="mt-4">
                    <x-input-label
                        for="notification_channels_discord_webhook_url"
                        :value="__('profile.notification_settings.fields.discord_webhook_url')"
                    />
                    <x-text-input
                        id="notification_channels_discord_webhook_url"
                        name="notification_channels[discord][webhook_url]"
                        type="url"
                        :value="data_get($notificationChannels, 'discord.webhook_url')"
            ...
            <div class="rounded-xl border border-gray-200 bg-gray-50/60 p-5 shadow-xs dark:border-gray-700 dark:bg-gray-900/30">
                <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <x-text-checkbox
                        id="notification_channels_teams_enabled"
                        name="notification_channels[teams][enabled]"
                        :checked="(bool) data_get($notificationChannels, 'teams.enabled', false)"
                        :label="__('profile.notification_settings.enabled')"
                    />
                    <x-secondary-button
                        form="test-teams-notification-channel"
                        :icon-only="true"
                        title="{{ __('profile.notification_settings.test.action') }}"
                        aria-label="{{ __('profile.notification_settings.test.action') }}"
                    >
                ...
                <div class="mt-4">
                    <x-input-label
                        for="notification_channels_teams_webhook_url"
                        :value="__('profile.notification_settings.fields.teams_webhook_url')"
                    />
                    <x-text-input
                        id="notification_channels_teams_webhook_url"
                        name="notification_channels[teams][webhook_url]"
                        type="url"
                        :value="data_get($notificationChannels, 'teams.webhook_url')"
                        placeholder="https://..."
                    />
            ...
            <div class="rounded-xl border border-gray-200 bg-gray-50/60 p-5 shadow-xs dark:border-gray-700 dark:bg-gray-900/30">
                <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <x-text-checkbox
                        id="notification_channels_webhook_enabled"
                        name="notification_channels[webhook][enabled]"
                        :checked="(bool) data_get($notificationChannels, 'webhook.enabled', false)"
                        :label="__('profile.notification_settings.enabled')"
                    />
                    <x-secondary-button
                        form="test-webhook-notification-channel"
                        :icon-only="true"
                        title="{{ __('profile.notification_settings.test.action') }}"
                        aria-label="{{ __('profile.notification_settings.test.action') }}"
                    >
                ...
                <div class="mt-4">
                    <x-input-label
                        for="notification_channels_webhook_url"
                        :value="__('profile.notification_settings.fields.webhook_url')"
                    />
                    <x-text-input
                        id="notification_channels_webhook_url"
                        name="notification_channels[webhook][url]"
                        type="url"
                        :value="data_get($notificationChannels, 'webhook.url')"
        ...
        <div class="rounded-xl border border-gray-200 bg-gray-50/60 p-5 shadow-xs dark:border-gray-700 dark:bg-gray-900/30">
            ...
            <x-text-checkbox
                id="monitoring_digest_enabled"
                name="monitoring_digest_enabled"
            ...
            <div class="mt-4 w-full md:w-1/2">
                <x-input-label
                    for="monitoring_digest_frequency"
                    :value="__('profile.notification_settings.digest.frequency')"
                />
                <x-select-input
                    id="monitoring_digest_frequency"
                    class="mt-1 block w-full"
                    name="monitoring_digest_frequency"
                >
                    @foreach (['daily', 'weekly', 'monthly'] as $frequency)
                        <option
                            value="{{ $frequency }}"
                            @selected(old('monitoring_digest_frequency', $user->monitoring_digest_frequency ?? 'weekly') === $frequency)
                        >
        ...
        <div class="rounded-xl border border-gray-200 bg-gray-50/60 p-5 shadow-xs dark:border-gray-700 dark:bg-gray-900/30">
            ...
            <x-text-checkbox
                id="unread_notifications_reminder_enabled"
                name="unread_notifications_reminder_enabled"
            ...
            <div class="mt-4 w-full md:w-1/2">
                <x-input-label
                    for="unread_notifications_reminder_frequency"
                    :value="__('profile.notification_settings.unread_reminder.frequency')"
                />
                <x-select-input
                    id="unread_notifications_reminder_frequency"
                    class="mt-1 block w-full"
                    name="unread_notifications_reminder_frequency"
                >
                    @foreach (['daily', 'weekly', 'monthly'] as $frequency)
                        <option
                            value="{{ $frequency }}"
                            @selected(old('unread_notifications_reminder_frequency', $user->unread_notifications_reminder_frequency ?? 'daily') === $frequency)
                        >
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        @if (! empty($modal))
            <x-secondary-button
                type="button"
                x-on:click="$dispatch('close-form-modal', 'profile-information-form-modal')"
            >
                {{ __('button.cancel') }}
            </x-secondary-button>
        @endif
...
@foreach ($notificationChannelKeys as $notificationChannelKey)
    <form
        id="test-{{ $notificationChannelKey }}-notification-channel"
        method="POST"
