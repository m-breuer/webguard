...
>
    <div class="flex min-h-16 items-center justify-between px-4 lg:h-auto lg:min-h-screen lg:flex-col lg:items-stretch lg:px-3 lg:py-5">
        <a
            href="{{ route('dashboard') }}"
            class="flex items-center gap-3 rounded-xl px-2 py-1.5 focus:ring-2 focus:ring-purple-300 focus:outline-hidden"
        ...
        >
            <span class="flex h-10 w-10 items-center justify-center rounded-xl bg-white p-1.5 shadow-sm">
                <img
                    src="{{ Vite::asset('resources/images/Logo-WebGuard.png') }}"
                    alt="{{ __('app.logo_alt') }}"
        ...
        >
            <button
                ...
                class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-purple-800 text-purple-200 transition hover:border-purple-600 hover:bg-purple-900 hover:text-white focus:ring-2 focus:ring-purple-300 focus:outline-hidden"
            >
                <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                    <path
                        ...
                        d="m14 6-6 6 6 6m-6-6h12"
                    />
                    <path
                        x-show="sidebarCollapsed"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        d="m10 6 6 6-6 6m6-6H4"
                    />
                </svg>
            </button>
        </div>
            ...
            >
                ...
                >
                    ...
                    @if (isset($unreadNotificationsCount) && $unreadNotificationsCount > 0)
                        <span class="absolute -top-1 -right-1 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-red-500 px-1.5 text-xs leading-none font-bold text-white">{{ $unreadNotificationsCount }}</span>
                    @endif
                </a>
                ...
                <x-dropdown align="right" placement="top" width="48" contentClasses="bg-white py-2 dark:bg-slate-900">
                    <x-slot name="trigger">
                        <button
                            ...
                            class="flex w-full items-center justify-between gap-3 rounded-xl border border-purple-800 px-3 py-2 text-left text-sm font-medium text-purple-100 transition hover:border-purple-600 hover:bg-purple-900 focus:ring-2 focus:ring-purple-300 focus:outline-hidden"
                            :class="{ 'lg:justify-center lg:px-0': sidebarCollapsed }"
                        >
                    ...
                    <x-slot name="content">
                        ...
                        <form method="POST" action="{{ route('logout') }}" id="logout-form-desktop">
                            ...
                            <x-dropdown-link
                                ...
                                "
                            >{{ __('button.logout') }}</x-dropdown-link>
                        </form>
                    </x-slot>
                </x-dropdown>
            </div>
        ...
        <div class="flex items-center gap-2 lg:hidden">
            <a
                ...
                aria-label="{{ __('notifications.title') }}"
                title="{{ __('notifications.title') }}"
                class="relative inline-flex h-10 w-10 items-center justify-center rounded-xl border border-purple-800 text-purple-100 transition hover:bg-purple-900 focus:ring-2 focus:ring-purple-300 focus:outline-hidden"
            >
            ...
            <button
                ...
                class="inline-flex items-center justify-center rounded-xl border border-purple-800 p-2 text-purple-100 transition hover:bg-purple-900 focus:ring-2 focus:ring-purple-300 focus:outline-hidden"
                aria-label="{{ __('app.navigation.home') }}"
            >
    ...
    <div
        ...
        x-show="mobileOpen"
        x-transition
        class="border-t border-purple-900/60 px-4 pt-3 pb-4 lg:hidden"
    >
        <p class="px-3 pb-2 text-xs font-semibold tracking-[0.16em] text-purple-300 uppercase">
            {{ __('app.navigation.dashboard') }}
        </p>
        <a
            href="{{ route('dashboard') }}"
            @class(['block w-full rounded-xl px-4 py-2.5 text-start text-base font-medium transition focus:outline-hidden focus:ring-2 focus:ring-purple-300', 'bg-purple-700 text-white' => request()->routeIs('dashboard'), 'text-purple-100 hover:bg-purple-900/70 hover:text-white' => ! request()->routeIs('dashboard')])
        >{{ __('app.navigation.dashboard') }}</a>
        ...
        <div class="mt-3 border-t border-purple-900/60 pt-3">
            @if (! Auth::user()->isDemo())
                <a
                    ...
                    class="block w-full rounded-xl px-4 py-2.5 text-start text-base font-medium text-purple-100 transition hover:bg-purple-900/70 hover:text-white focus:ring-2 focus:ring-purple-300 focus:outline-hidden"
                >{{ __('profile.title') }}</a>
            @endif
