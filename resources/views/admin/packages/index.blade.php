<x-app-layout>
    <x-slot name="header">
        <x-heading type="h1"> {{ __('admin.packages.title') }} </x-heading>
    </x-slot>

    <x-main>
        <div x-data="formModalLoader()" data-form-modal-error="{{ __('app.messages.form_modal_load_error') }}">
            <x-async-table
                id="admin-packages-table"
                :endpoint="route('admin.packages.index')"
                :paginator="$packages"
                :filters="$filters"
            ...
            >
                ...
                <x-slot name="head">
                    <x-table.heading sort="monitoring_limit">
                        {{ __('admin.packages.fields.monitoring_limit') }}</x-table.heading>
                    <x-table.heading sort="price">{{ __('admin.packages.fields.price') }}</x-table.heading>
                    <x-table.heading sort="is_selectable">
                        {{ __('admin.packages.fields.is_selectable') }}</x-table.heading>
            ...
            <x-form-modal
                name="admin-package-form-modal"
                title="{{ __('admin.packages.title') }}"
