...
<section
    class="rounded-lg border border-gray-200 bg-gray-50/70 p-4 shadow-none sm:p-5 dark:border-gray-700 dark:bg-gray-900/30"
    ...
    <div @class(['mt-4 space-y-4' => $embedded, 'space-y-4' => ! $embedded])>
        @if ($embedded)
            <label for="{{ $fieldIdPrefix }}_notification_on_failure" class="inline-flex items-center">
                <input
                    id="{{ $fieldIdPrefix }}_notification_on_failure"
                    name="notification_on_failure"
                    type="checkbox"
                    value="1"
                    form="{{ $formId }}"
                    class="focus:ring-opacity-50 rounded-sm border-gray-300 text-purple-600 shadow-xs focus:border-purple-300 focus:ring-3 focus:ring-purple-200 dark:border-gray-600"
        ...
        <div>
            <x-input-label
                for="{{ $fieldIdPrefix }}_notification_channels"
                :value="__('monitoring.form.notification_channels')"
            />
            ...
            @else
                <div
                    id="{{ $fieldIdPrefix }}_notification_channels"
                    class="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3"
                >
        ...
        <div>
            <x-input-label
                for="{{ $fieldIdPrefix }}_ssl_expiry_warning_days"
                :value="__('monitoring.form.ssl_expiry_warning_days')"
            />
            <input
                id="{{ $fieldIdPrefix }}_ssl_expiry_warning_days"
                type="number"
                min="1"
                max="365"
                name="ssl_expiry_warning_days"
                value="{{ old('ssl_expiry_warning_days', $notificationPreference->ssl_expiry_warning_days) }}"
                @if ($embedded) form="{{ $formId }}" @endif
        ...
        @if ($embedded)
            <x-secondary-button form="{{ $formId }}"> {{ __('button.save') }} </x-secondary-button>
        @else
            <x-secondary-button> {{ __('button.save') }} </x-secondary-button>
        @endif
    </div>
    @unless ($embedded)
        </form>
