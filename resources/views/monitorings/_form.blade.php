...
>
    <div class="space-y-8">
            ...
            <div>
                ...
                @if (isset($monitoring))
                    <x-text-input
                        ...
                        name="type"
                        :value="__('monitoring.types.' . $monitoring->type->value)"
                        readonly
                    />
        ...
        <details class="space-y-4 border-t border-gray-200 pt-6 dark:border-gray-700">
            <summary class="group [&::-webkit-details-marker]:hidden flex cursor-pointer list-none items-center justify-between gap-4">
                <x-heading type="h2">{{ __('monitoring.form.sections.organization') }}</x-heading>
                <svg
                    class="size-5 shrink-0 text-gray-500 transition-transform group-open:rotate-180 dark:text-gray-400"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                    aria-hidden="true"
                >
            ...
            <div>
                ...
                @if (isset($monitoring))
                    <x-text-input
                        id="team_id"
                        class="cursor-not-allowed"
                        :value="$monitoring->team ? __('team.ownership.team') . ': ' . $monitoring->team->name : __('team.ownership.private')"
                        readonly
                    />
                @else
                ...
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    @if (! Auth::user()->isDemo())
                        <a
                            ...
                            class="text-purple-700 underline dark:text-purple-300"
                        >
        ...
        <details
            data-monitoring-check-configuration
            data-monitoring-type-fields="{{ implode(' ', $checkConfigurationTypeValues) }}"
            {{ $showCheckConfiguration ? '' : 'hidden' }}
            ...
            <div class="space-y-4 pt-4">
                ...
                >
                    ...
                    <x-text-input
                        ...
                        type="text"
                        name="keyword"
                        :value="old('keyword', $monitoring->keyword ?? '')"
                    />
                ...
                >
                    <div>
                        <x-input-label
                            for="heartbeat_interval_minutes"
                            :value="__('monitoring.form.heartbeat_interval_minutes')"
                        />
                        <x-text-input
                            ...
                            :value="old('heartbeat_interval_minutes', $monitoring->heartbeat_interval_minutes ?? 60)"
                        />
                        <x-input-error :messages="$errors->get('heartbeat_interval_minutes')" />
                    </div>
                    <div>
                        <x-input-label
                            for="heartbeat_grace_minutes"
                            :value="__('monitoring.form.heartbeat_grace_minutes')"
                        />
                        <x-text-input
                            id="heartbeat_grace_minutes"
                            type="number"
                            min="0"
                            max="1440"
                            name="heartbeat_grace_minutes"
                            :value="old('heartbeat_grace_minutes', $monitoring->heartbeat_grace_minutes ?? 5)"
                        />
                        <x-input-error :messages="$errors->get('heartbeat_grace_minutes')" />
                    </div>
                    <p class="text-sm text-gray-600 md:col-span-2 dark:text-gray-400">
                        {{ __('monitoring.form.heartbeat_help') }}
                    </p>
                </div>

                <div
                    data-monitoring-type-fields="{{ MonitoringType::SERVER_HEALTH->value }}"
                    {{ $selectedType !== MonitoringType::SERVER_HEALTH->value ? 'hidden' : '' }}
                ...
                >
                    ...
                    <div>
                        <x-heading
                            type="h3"
                            class="text-base"
                        >{{ __('monitoring.form.server_health_thresholds') }}</x-heading>
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                        <div>
                            <x-input-label
                                for="server_health_cpu_threshold_percent"
                                :value="__('monitoring.form.server_health_cpu_threshold_percent')"
                            />
                            <x-text-input
                                id="server_health_cpu_threshold_percent"
                                type="number"
                                min="1"
                                max="100"
                                step="0.01"
                                name="server_health_cpu_threshold_percent"
                                :value="old('server_health_cpu_threshold_percent', isset($monitoring) ? $monitoring->server_health_cpu_threshold_percent : 90)"
                            />
                            <x-input-error :messages="$errors->get('server_health_cpu_threshold_percent')" />
                        </div>
                        <div>
                            <x-input-label
                                for="server_health_ram_threshold_percent"
                                :value="__('monitoring.form.server_health_ram_threshold_percent')"
                            />
                            <x-text-input
                                id="server_health_ram_threshold_percent"
                                type="number"
                                min="1"
                                max="100"
                                step="0.01"
                                name="server_health_ram_threshold_percent"
                                :value="old('server_health_ram_threshold_percent', isset($monitoring) ? $monitoring->server_health_ram_threshold_percent : 90)"
                            />
                            <x-input-error :messages="$errors->get('server_health_ram_threshold_percent')" />
                        </div>
                        <div>
                            <x-input-label
                                for="server_health_load_threshold_per_cpu"
                                :value="__('monitoring.form.server_health_load_threshold_per_cpu')"
                            />
                            <x-text-input
                                id="server_health_load_threshold_per_cpu"
                                type="number"
                                min="0.01"
                                max="100"
                                step="0.01"
                                name="server_health_load_threshold_per_cpu"
                                :value="old('server_health_load_threshold_per_cpu', isset($monitoring) ? $monitoring->server_health_load_threshold_per_cpu : '')"
                            />
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                        <div>
                            <x-input-label
                                for="server_health_service_response_time_threshold_ms"
                                :value="__('monitoring.form.server_health_service_response_time_threshold_ms')"
                            />
                            <x-text-input
                                id="server_health_service_response_time_threshold_ms"
                                type="number"
                                min="1"
                                max="60000"
                                name="server_health_service_response_time_threshold_ms"
                                :value="old('server_health_service_response_time_threshold_ms', isset($monitoring) ? $monitoring->server_health_service_response_time_threshold_ms : '')"
                            />
                            <x-input-error :messages="$errors->get('server_health_service_response_time_threshold_ms')" />
                        </div>
                        <div>
                            <x-input-label
                                for="server_health_report_interval_minutes"
                                :value="__('monitoring.form.server_health_report_interval_minutes')"
                            />
                            <x-text-input
                                id="server_health_report_interval_minutes"
                                type="number"
                                min="1"
                                max="1440"
                                name="server_health_report_interval_minutes"
                                :value="old('server_health_report_interval_minutes', isset($monitoring) ? $monitoring->server_health_report_interval_minutes : 1)"
                            />
                            <x-input-error :messages="$errors->get('server_health_report_interval_minutes')" />
                        </div>
                        <div>
                            <x-input-label
                                for="server_health_grace_minutes"
                                :value="__('monitoring.form.server_health_grace_minutes')"
                            />
                            <x-text-input
                                id="server_health_grace_minutes"
                                type="number"
                                min="0"
                                max="1440"
                                name="server_health_grace_minutes"
                                :value="old('server_health_grace_minutes', isset($monitoring) ? $monitoring->server_health_grace_minutes : 5)"
                            />
                ...
                <div
                    data-monitoring-type-fields="{{ MonitoringType::DNS_RECORD->value }}"
                    {{ $selectedType !== MonitoringType::DNS_RECORD->value ? 'hidden' : '' }}
                    class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2"
                >
                    <div>
                        ...
                        <x-select-input id="dns_record_type" class="mt-1 block w-full" name="dns_record_type">
                            @foreach (\App\Support\DnsRecordExpectation::recordTypes() as $recordType)
                                <option
                                    value="{{ $recordType }}"
                                    @selected(old('dns_record_type', $monitoring->dns_record_type ?? 'A') === $recordType)
                                >
                                    {{ $recordType }}
                                </option>
                            @endforeach
                    ...
                    <div class="md:col-span-2">
                        ...
                        <x-textarea
                            id="dns_expected_values"
                            name="dns_expected_values"
                            rows="5"
                            placeholder="{{ __('monitoring.form.placeholders.dns_expected_values') }}"
                        >{{ $dnsExpectedValues ?? '' }}</x-textarea>
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                >
                    <div>
                        <x-input-label
                            for="response_time_threshold_ms"
                            :value="__('monitoring.form.response_time_threshold_ms')"
                        />
                        <x-text-input
                            ...
                            :value="old('response_time_threshold_ms', isset($monitoring) ? $monitoring->response_time_threshold_ms : null)"
                            class="mt-1 block w-full"
                        />
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                            {{ __('monitoring.form.response_time_threshold_ms_help') }}
                        </p>
                        <x-input-error :messages="$errors->get('response_time_threshold_ms')" />
                    </div>
                    <div>
                        <x-input-label
                            for="response_time_confirmation_threshold"
                            :value="__('monitoring.form.response_time_confirmation_threshold')"
                        />
                        <x-text-input
                            id="response_time_confirmation_threshold"
                            type="number"
                            min="1"
                            max="10"
                            name="response_time_confirmation_threshold"
                            :value="old('response_time_confirmation_threshold', isset($monitoring) ? $monitoring->response_time_confirmation_threshold : null)"
                            class="mt-1 block w-full"
                        />
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                            {{ __('monitoring.form.response_time_confirmation_threshold_help') }}
                        </p>
                ...
                >
                    ...
                    <x-select-input id="http_method" class="mt-1 block w-full" name="http_method">
                        @foreach (HttpMethod::cases() as $method)
                            <option
                                value="{{ $method->value }}"
                                @selected(old('http_method', $monitoring->http_method?->value ?? 'GET') === $method->value)
                            >
                ...
                >
                    ...
                    <x-text-input
                        id="expected_http_statuses"
                        type="text"
                        name="expected_http_statuses"
                        :value="old('expected_http_statuses', $monitoring->expected_http_statuses ?? '200-299')"
                ...
                >
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input
                            ...
                            type="text"
                            name="auth_username"
                            :value="old('auth_username', $monitoring->auth_username ?? '')"
                        />
                        ...
                        <x-text-input
                            ...
                            type="password"
                            name="auth_password"
                            :value="old('auth_password', $monitoring->auth_password ?? '')"
                        />
                    ...
                    <div class="mt-4">
                        ...
                        <x-textarea
                            ...
                            name="http_headers"
                            rows="4"
                            placeholder="{{ __('monitoring.form.placeholders.http_headers') }}"
                        >{{ $httpHeadersValue ?? '' }}</x-textarea>
                    ...
                    <div class="mt-4">
                        ...
                        <x-textarea
                            ...
                            name="http_body"
                            rows="4"
                            placeholder="{{ __('monitoring.form.placeholders.http_body') }}"
                        >{{ old('http_body', $monitoring->http_body ?? '') }}</x-textarea>
        ...
        <details class="space-y-4 border-t border-gray-200 pt-6 dark:border-gray-700">
            ...
            <div class="mt-4">
                ...
                <label class="relative inline-flex cursor-pointer items-center">
                    <input
                        type="checkbox"
                        name="public_label_enabled"
                        value="1"
                        class="peer sr-only"
                        x-model="publicLabelEnabled"
                        @if (old('public_label_enabled', $monitoring->public_label_enabled ?? false)) checked @endif
                    />
                    ...
                    <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('monitoring.form.public_label_enabled') }}</span>
                </label>
                @if (isset($monitoring))
                    <div x-show="publicLabelEnabled" x-transition>
                        ...
                        <div class="mt-4">
                            ...
                            <x-paragraph class="text-sm text-gray-600 dark:text-gray-400">{{ __('monitoring.detail.sla_badge.description') }}</x-paragraph>
                            <x-paragraph class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('monitoring.detail.sla_badge.snippet_help') }}</x-paragraph>
                            <div class="mt-2 flex items-center space-x-2">
                                <pre
                                    id="sla-badge-snippet"
                                    class="focus:ring-opacity-50 flex-grow overflow-auto rounded-md border-gray-300 bg-gray-100 p-2 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200"
                                ><code>&lt;div data-webguard-sla-badge data-monitoring="{{ $monitoring->id }}" data-range="90" data-theme="auto" data-size="compact"&gt;&lt;/div&gt;
&lt;script src="{{ route('badge.js') }}"&gt;&lt;/script&gt;</code></pre>
        ...
        <details class="space-y-4 border-t border-gray-200 pt-6 dark:border-gray-700">
            ...
            @unless (isset($monitoring))
                <div class="mt-4">
                    <x-input-label
                        for="notification_on_failure"
                        :value="__('monitoring.form.notification_on_failure')"
                    />
                    <label class="relative inline-flex cursor-pointer items-center">
                        <input
                            type="checkbox"
                            ...
                            value="1"
                            class="peer sr-only"
                            x-model="notificationOnFailure"
                            @if (old('notification_on_failure', $monitoring->notification_on_failure ?? true)) checked @endif
                        />
                        <div class="peer h-6 w-11 rounded-full bg-gray-200 peer-checked:bg-purple-600 peer-focus:ring-4 peer-focus:ring-purple-300 peer-focus:outline-none after:absolute after:top-[2px] after:left-[2px] after:h-5 after:w-5 after:rounded-full after:border after:border-gray-300 after:bg-white after:transition-all after:content-[''] peer-checked:after:translate-x-full peer-checked:after:border-white dark:border-gray-600 dark:bg-gray-700 dark:peer-focus:ring-purple-800"></div>
                        <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('monitoring.form.notification_on_failure_enabled') }}</span>
                    </label>
                </div>
            @endunless
            ...
            <div class="mt-4">
                <x-input-label
                    for="failure_confirmation_threshold"
                    :value="__('monitoring.form.failure_confirmation_threshold')"
                />
                <x-text-input
                    id="failure_confirmation_threshold"
                    type="number"
                    min="1"
                    max="10"
                    name="failure_confirmation_threshold"
                    :value="old('failure_confirmation_threshold', $monitoring->failure_confirmation_threshold ?? 1)"
                />
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            @unless (isset($monitoring))
                <div class="mt-4">
                    ...
                    @if (count($enabledNotificationChannels) > 0)
                        <x-select-input
                            ...
                            multiple
                            size="{{ min(4, count($enabledNotificationChannels)) }}"
                            class="mt-1 block w-full"
                        >
                            @foreach ($enabledNotificationChannels as $channel)
                                <option
                                    value="{{ $channel }}"
                                    @selected(in_array($channel, $selectedNotificationChannels, true))
                                >
                ...
                <div class="mt-4">
                    <x-input-label
                        for="ssl_expiry_warning_days"
                        :value="__('monitoring.form.ssl_expiry_warning_days')"
                    />
                    <x-text-input
            ...
            <div class="mt-4">
                ...
                <x-select-input id="status" class="mt-1 block w-full" name="status" required>
                    @foreach (MonitoringLifecycleStatus::cases() as $status)
                        <option
                            value="{{ $status->value }}"
                            @selected(old('status', $monitoring->status?->value ?? 'active') === $status->value)
                        >
        ...
        <div
            class="flex flex-wrap justify-end gap-2 border-t border-gray-200 pt-6 dark:border-gray-700"
