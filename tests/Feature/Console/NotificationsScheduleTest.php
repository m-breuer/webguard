...
class NotificationsScheduleTest extends TestCase
{
    ...
    public function test_infrastructure_heartbeat_is_scheduled_every_minute_without_overlap(): void
    {
        /** @var Event|null $event */
        $event = collect(resolve(Schedule::class)->events())
            ->first(fn (Event $event): bool => str_contains((string) $event->command, 'infrastructure:heartbeat'));
