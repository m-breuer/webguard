...
<main class="mx-auto w-[min(64rem,calc(100%_-_2rem))] py-6 sm:py-12">
    <header class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="m-0 text-[0.8125rem] font-extrabold tracking-[0.1em] text-wg-accent uppercase">Monitoring detail</p><h1 class="mt-2 text-[clamp(2rem,6vw,3rem)] leading-[1.1] font-bold">{data.monitoring.name}</h1><p class="mt-3 break-all leading-6 text-wg-text-muted">{data.monitoring.target}</p></div>{#if data.monitoring.can_manage}<div class="flex flex-wrap gap-3 md:flex-nowrap"><a class="inline-flex min-h-11 items-center rounded-md border border-wg-border px-4 py-2.5 text-sm font-bold text-wg-text no-underline" href={`/monitorings/${data.monitoring.id}/edit`}>Edit</a><Button variant="danger" type="button" disabled={deleting} onclick={remove}>Delete</Button></div>{/if}</header>
