<script lang="ts">
    ...
    interface Props { title: string; description: string; singular: string; endpoint: string; items: Item[]; columns: Column[]; fields: CrudField[]; createDefaults?: Item; itemLabel?: (item: Item) => string; extraActions?: (item: Item) => Array<{ label: string; endpoint: string }>; }
