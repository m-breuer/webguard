...
<form class="grid gap-6" onsubmit={submit} novalidate>
    ...
    <section class="grid gap-4 border-t border-wg-border pt-6">
        ...
        {/if}
        {#if httpTypes}
            <div class="grid gap-4 sm:grid-cols-2"><Field label="HTTP method" error={errorFor("http_method")}><Select name="http_method" value={monitoring?.http_method ?? "get"}>{#each ["get", "post", "put", "patch", "delete"] as httpMethod}<option value={httpMethod}>{httpMethod.toUpperCase()}</option>{/each}</Select></Field><Field label="Timeout (seconds)" error={errorFor("timeout")} required><Input name="timeout" type="number" min="1" max="60" value={monitoring?.timeout ?? 5} required /></Field><Field label="Expected HTTP statuses" error={errorFor("expected_http_statuses")}><Input name="expected_http_statuses" value={monitoring?.expected_http_statuses ?? "200-299"} /></Field>{#if type === "keyword"}<Field label="Required keyword" error={errorFor("keyword")} required><Input name="keyword" value={monitoring?.keyword ?? ""} required /></Field>{/if}</div>
    ...
    <section class="grid gap-4 border-t border-wg-border pt-6">
        ...
        {#if !monitoring && options.teams.length > 0}<Field label="Owner"><Select name="team_id"><option value="">Private monitoring</option>{#each options.teams as team}<option value={team.id}>Team: {team.name}</option>{/each}</Select></Field>{/if}
        <fieldset class="grid gap-2"><legend class="text-sm font-bold">Monitoring locations</legend>{#each options.locations as location}<label class="flex items-center gap-2 text-sm"><Checkbox name="preferred_locations[]" value={location} bind:group={selectedLocations} />{location}</label>{/each}{#if errorFor("preferred_locations")}<p class="text-sm text-wg-danger">{errorFor("preferred_locations")}</p>{/if}</fieldset>
        {#if !monitoring || monitoring.can_assign_groups}<fieldset class="grid gap-2"><legend class="text-sm font-bold">Groups</legend>{#each options.groups as group}<label class="flex items-center gap-2 text-sm"><Checkbox name="group_ids[]" value={group.id} bind:group={selectedGroups} />{group.name}</label>{/each}</fieldset>{/if}
